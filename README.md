...
a tab delimited three column file containing:  
 
 * column1: name of the haplotype/species   

 * column2: name of the sex/mating type chromosome(s)  


 * column3: whether the chromosome(s)/contig(s) is(are) kept in Normal order (N) or reversed (R)   

see example [here](https://github.com/QuentinRougemont/EASYstrata/blob/main/example_data/scaffold.txt)
...
## 8: working from existing single copy orthologs matrix (possibly from many individuals/species)

Use case:
 when a run of orthofinder was performed and included many species or sample. 

 this can reduce the number of false positive single copy orthologs 
